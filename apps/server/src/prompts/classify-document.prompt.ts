const CLASSIFY_DOCUMENT_BASE_PROMPT = `
You are an AI document classifier. Your job is to classify the uploaded document.
...
const CLASSIFY_DOCUMENT_TEXT_PROMPT_TEMPLATE = `
...
export function buildClassifyPrompt(ocrText: string): string {
  return CLASSIFY_DOCUMENT_TEXT_PROMPT_TEMPLATE.replace("{{OCR_TEXT}}", ocrText);
}

/** @deprecated Use buildClassifyPrompt(ocrText) instead. Kept for backward compatibility with buffer-based classification. */
export const CLASSIFY_DOCUMENT_PROMPT = CLASSIFY_DOCUMENT_BASE_PROMPT.trim();
